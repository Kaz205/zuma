				      ...
				      const char *buf, size_t count)
{
	...
	unsigned int rate_limit_us, max_rate_limit_us;
	int cpu;
	...
	max_rate_limit_us = max(rate_limit_us, max(tunables->up_rate_limit_us,
					   tunables->down_rate_limit_us));
	rate_limit_util = approximate_util_avg(0, max_rate_limit_us + SHORT_TASK_MIN);

	list_for_each_entry(sg_policy, &attr_set->policy_list, tunables_hook) {
		sg_policy->up_rate_delay_ns = rate_limit_us * NSEC_PER_USEC;
		update_min_rate_limit_ns(sg_policy);
		for_each_cpu(cpu, sg_policy->policy->cpus) {
			per_cpu(dvfs_update_delay, cpu) = max_rate_limit_us;
...
static int sugov_start(struct cpufreq_policy *policy)
{
	...
	unsigned long rate_limit_util;
	unsigned int cpu, max_rate_limit_us;
	...
	max_rate_limit_us = max(sg_policy->tunables->down_rate_limit_us,
				   sg_policy->tunables->up_rate_limit_us);
	rate_limit_util = approximate_util_avg(0, max_rate_limit_us + SHORT_TASK_MIN);
